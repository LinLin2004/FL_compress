...
def plot_figure(data_iid, iid_label, save_path):
    ...
    for ax_idx, attack in enumerate(attacks):
        ...
        for i, ratio_raw in enumerate(all_ratios):
            if ratio_raw not in attack_data:
                continue
            acc_list = attack_data[ratio_raw]
            ...
            ax.plot(
                ...
                acc_list,
                color=color,
                label=f'Byzantine Ratio={format_ratio(ratio_raw)}',
                linewidth=1.5
            )
            ...
            ax.plot(
                sample_idx,
                [acc_list[j] for j in sample_idx],
                color=color,
                marker=marker,
                markersize=6,
                linestyle='None'
    ...
    plt.suptitle(f'Byzantine Ratio ({iid_label})', fontsize=20, y=1.02)
    plt.tight_layout()
