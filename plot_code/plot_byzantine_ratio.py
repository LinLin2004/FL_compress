...
ROOT_DIR = '/data-store/xulihan/FL_compress/results/byzantine_ratio'


# Parse Byzantine ratio from dirname like:
# cifar10_adamk_compress4_clip_k469_r2_byzantine04_foe_iid
def parse_dirname(dirname):
    m = re.match(
        r'cifar10_adamk_compress4_clip_k\d+_r\d+_byzantine(\d+)_(\w+)_(iid|noniid)',
        dirname
    )
    if not m:
        return None
    ...
    attack = m.group(2)
    iid_flag = m.group(3)
    return ratio_raw, attack, iid_flag


def format_ratio(ratio_raw):
    return str(int(ratio_raw) / 10)
...
def read_acc_list(folder_path):
    ...
    return [a['top1_accuracy'] for a in acc_list if 'top1_accuracy' in a]


def organize_data(root_dir):
    """Returns: data[iid_flag][attack][ratio_raw] = acc_list"""
    data = {}
    ...
    for subdir in subdirs:
        ...
        if parsed is None:
            ...
            continue
        ratio_raw, attack, iid_flag = parsed
        ...
        data[iid_flag][attack][ratio_raw] = acc_list
        print(
            f"[INFO] Loaded: iid={iid_flag}, attack={attack}, "
            f"byzantine_ratio={format_ratio(ratio_raw)}, epochs={len(acc_list)}"
        )
    return data
...
def sort_ratio(ratio_list):
    return sorted(ratio_list, key=lambda x: int(x))
...
def _plot_attack_ridge(ax, attack_data, all_values, value_to_color, value_prefix, y_label):
    ...
    ax.set_yticklabels([f'{value_prefix}={format_ratio(val)}' for val in available_values])
...
def plot_figure(data_iid, iid_label, save_path):
    ...
    for attack in attacks:
        all_ratios.update(data_iid[attack].keys())
    all_ratios = sort_ratio(list(all_ratios))

    value_to_color = build_value_colors(all_ratios)
    ...
    for ax_idx, attack in enumerate(ATTACK_ORDER):
        ...
        if attack in data_iid:
            _plot_attack_ridge(ax, data_iid[attack], all_ratios, value_to_color,
                               'ratio', 'Byzantine Ratio')
            ...
            legend_handles = [
                Line2D([0], [0], marker='o', linestyle='None',
                       markerfacecolor=value_to_color[ratio_raw],
                       markeredgecolor='none', markersize=6.5,
                       label=f'ratio={format_ratio(ratio_raw)}')
                for ratio_raw in all_ratios
                if ratio_raw in data_iid[attack]
            ]
...
if __name__ == '__main__':
    ...
    for iid_flag in ['iid', 'noniid']:
        ...
        save_path = os.path.join(ROOT_DIR, f'byzantine_ratio_{iid_flag}.png')
