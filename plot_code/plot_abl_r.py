...
def plot_figure(data_iid, iid_label, save_path):
    ...
    for ax_idx, attack in enumerate(attacks):
        ...
        for i, r_val in enumerate(all_r):
            if r_val not in attack_data:
                continue
            acc_list = attack_data[r_val]
            xvals = np.arange(len(acc_list))
            color = COLORS[i % len(COLORS)]
            marker = MARKERS[i % len(MARKERS)]
            ax.plot(xvals, acc_list, color=color, label=f'r={r_val}', linewidth=1.5)
    ...
    plt.suptitle(f'Ablation on r ({iid_label})', fontsize=20, y=1.02)
    plt.tight_layout()
