...
def plot_figure(data_iid, iid_label, save_path):
    ...
    value_to_color = build_value_colors(all_bs)
    ...
    for ax_idx, attack in enumerate(ATTACK_ORDER):
        ...
        if attack in data_iid:
            _plot_attack_ridge(ax, data_iid[attack], all_bs, value_to_color, 'BS', 'Batch Size')
            ...
            legend_handles = [
                Line2D([0], [0], marker='o', linestyle='None',
                       markerfacecolor=value_to_color[bs_val],
                       markeredgecolor='none', markersize=6.5,
                       label=f'BS={bs_val}')
                for bs_val in all_bs
                if bs_val in data_iid[attack]
            ]
