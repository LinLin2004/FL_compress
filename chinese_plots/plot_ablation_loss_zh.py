...
# ===================== 绘图 =====================
def plot_ablation(data, config, output_dir, ylabel, ylim):
    if not data:
        return
    ...
    for attack, attack_data in data.items():
        for agg_name in aggregate_map.values():
            if agg_name not in attack_data:
                continue
            for optimizer, loss_list in attack_data[agg_name].items():
    ...
    plt.tight_layout()
    save_path = os.path.join(output_dir, f"ablation_{config['name']}_loss.pdf")
...
if __name__ == '__main__':
    ...
    for config in ABLATION_CONFIGS:
        ...
        if data:
            plot_ablation(data, config, OUTPUT_DIR, '损失', (0.5, 2.5))
        else:
