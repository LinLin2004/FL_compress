...
# ===================== 读取数据 =====================
def read_loss_data(folder_path):
    ...
    loss_list = checkpoint.get('history', {}).get('loss', [])
    return loss_list
...
def organize_data(root_dir):
    ...
    for subdir in subdirs:
        ...
        loss_list = read_loss_data(os.path.join(root_dir, subdir))
        if not loss_list:
            continue

        data.setdefault(attack, {}).setdefault(agg_name, {})[opt_name] = loss_list
        print(f"[INFO] Loaded: attack={attack}, aggregate={agg_name}, optimizer={opt_name}, epochs={len(loss_list)}")
...
# ===================== 绘图 =====================
def plot_comparison(data, output_dir, basename, ylabel, ylim):
    ...
    for group_idx, group_attacks in enumerate(groups):
        fig, axes = plt.subplots(1, len(group_attacks), figsize=(8 * len(group_attacks), 6), dpi=300)
        ...
        for ax, attack in zip(axes, group_attacks):
            ...
            for aggregate in AGGREGATE_NAME_MAP.values():
                ...
                for optimizer in OPTIMIZER_NAME_MAP.values():
                    ...
                    loss_list = attack_data[aggregate][optimizer]
                    if len(loss_list) > max_epochs:
                        max_epochs = len(loss_list)
...
if __name__ == '__main__':
    ...
    plot_comparison(data, OUTPUT_DIR, 'comparison_loss', '损失', (0.5, 2.5))
